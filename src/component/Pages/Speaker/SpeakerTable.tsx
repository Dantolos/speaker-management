...
import {
  ...
  X,
  Eye,
  SquareArrowOutUpRight, // ← statt SquarePen, semantisch passender
} from "lucide-react";
import ContactDrawer from "@/component/UI/ContactDrawer";
        ...
        <div className="flex flex-wrap gap-3 mb-4 items-center">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 rounded-xl border border-primary bg-primary/20 text-sm text-primary hover:bg-gray-50 transition-colors"
            >
              <X size={14} />
              Clear
            </button>
            ...
            <tbody>
              {speakers.length === 0 ? (
                ...
                speakers.map((speaker, i) => {
                  const contactId = speaker.Person?.[0];
                  return (
                    <tr
                      key={speaker.id ?? i}
                      onClick={() => {
                      ...
                      className={`border-b border-foreground/10 last:border-0 transition-colors ${
                        contactId ? "hover:bg-primary/10 cursor-pointer" : ""
                      }`}
                    >
                      ...
                      <td className="px-4 py-3 text-foreground/60">
                        {speaker["Speakerformular_Status"] ===
                          "Eingereicht" && (
                          <div className="h-3 w-3 rounded-2xl bg-green-400" />
                        )}
                        {speaker["Speakerformular_Status"] ===
                          "Nicht eingereicht" && (
                          <div className="h-3 w-3 rounded-2xl bg-red-700" />
                        )}
                        {!speaker["Speakerformular_Status"] && (
                          <div className="h-3 w-3 rounded-2xl bg-foreground/20" />
                        )}
                      </td>
                      <td className="px-4 py-3 text-right">
                        <Link
                          ...
                          onClick={(e) => e.stopPropagation()}
                          className="text-xs font-medium text-primary/60 hover:text-primary hover:scale-110 transition-all duration-200 inline-block"
                          title="Öffentliches Dossier öffnen"
                        >
                          <Eye />
                        </Link>
                      </td>
